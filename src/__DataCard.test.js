...
const responses = [
	...
	{ data: { altitude: 173, last_updated: "2020-09-23T02:20:07.860Z" } },
	{ data: { altitude: 123, last_updated: "2020-09-23T02:20:17.860Z" } },
...
describe("<DataCard />", () => {
	...
	it("accepts and calculates Average Altitude data from api", async () => {
		const { getByText } = render(<TestDataCardComponent />);
		await wait();
		expect(getByText("Current Altitude: 173 km")).toBeInTheDocument();
		await wait();
		expect(getByText("Average Altitude: 173 km")).toBeInTheDocument();
	});
});
